...
export function SignalDetailPage() {
  ...
  return <main className="content-wrap grid gap-8 py-12 lg:grid-cols-[1fr_360px]">
    <article className="panel p-7 md:p-10"><div className="flex flex-wrap items-center gap-3"><p className="eyebrow">Signal</p><span className="status-chip">{signal.state}</span></div><h1 className="font-display mt-4 text-6xl uppercase">{signal.title}</h1><p className="mt-3 font-mono text-sm text-neon-cyan">From {signal.sender.name}</p>{signal.body && <p className="mt-8 whitespace-pre-wrap text-lg leading-8 text-foreground-secondary">{signal.body}</p>}{signal.target?.title && <div className="mt-8 border-l-2 border-neon-purple pl-4"><p className="eyebrow">Attached to</p><p className="mt-1">{signal.target.title}</p></div>}</article>
