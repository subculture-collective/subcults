...
export function TourDetailPage() {
  ...
  return <main><PageMeta title={data.tour?.title ?? 'Tour dates'}/>
    ...
    <div className="content-wrap grid gap-8 py-10 lg:grid-cols-[1.15fr_.85fr]">
      ...
      <section aria-labelledby="tour-dates"><p className="eyebrow">Itinerary</p><h2 id="tour-dates" className="font-display mt-2 text-4xl uppercase">Tour dates</h2><div className="mt-5 grid max-h-[620px] gap-4 overflow-y-auto pr-1">{data.appearances.map((appearance) => <AppearanceCard key={appearance.id} appearance={appearance} />)}{data.appearances.length === 0 && <div className="panel p-7 text-foreground-secondary">No tour dates announced yet.</div>}</div></section>
    </div>
    {selected.length > 0 && <aside className="fixed inset-x-4 bottom-4 z-30 panel mx-auto max-w-2xl p-5 shadow-2xl" aria-live="polite"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Selected date</p><h2 className="font-display mt-1 text-3xl uppercase">{selected[0].event.title}</h2></div><button className="button-secondary" onClick={() => setSelectedEventID(null)}>Close</button></div><p className="mt-3 text-sm text-foreground-secondary">{selected.length} billed appearance{selected.length === 1 ? '' : 's'} at this event.</p></aside>}
  </main>;
}
