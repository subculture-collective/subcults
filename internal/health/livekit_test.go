...
// TestLiveKitChecker_WebSocketURL verifies that the configured WebSocket URL
// is normalized to HTTP for the reachability probe.
func TestLiveKitChecker_WebSocketURL(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	defer server.Close()

	checker := NewLiveKitChecker(strings.Replace(server.URL, "http://", "ws://", 1))
	if err := checker.HealthCheck(context.Background()); err != nil {
		t.Fatalf("expected WebSocket URL health check to succeed, got %v", err)
	}
}
