...
func (l *LiveKitChecker) HealthCheck(ctx context.Context) error {
	...
	healthURL := l.url
	if strings.HasPrefix(healthURL, "wss://") {
		healthURL = "https://" + strings.TrimPrefix(healthURL, "wss://")
	} else if strings.HasPrefix(healthURL, "ws://") {
		healthURL = "http://" + strings.TrimPrefix(healthURL, "ws://")
	}
